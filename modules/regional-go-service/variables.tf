...
variable "custom_audiences" {
  type        = list(string)
  description = <<EOD
Forwarded to regional-service. Optional list of custom audiences accepted by the
Cloud Run service's ID-token validation, in addition to the *.run.app URL. Use
for services reached by a non-run.app hostname or IP (e.g. behind an internal
HTTP ALB / Private Service Connect). Empty leaves custom audiences unset.
EOD
  default     = []
}
...
variable "raw_containers" {
  description = "Additional containers to run from prebuilt images rather than built from Go source — e.g. a cloud-sql-proxy sidecar. Same shape as regional-service's containers; keys must not collide with var.containers. Images are not cosign-signed by this module, so pin them by digest or a version tag you trust."
  type = map(object({
    image   = string
    command = optional(list(string), [])
    args    = optional(list(string), [])
