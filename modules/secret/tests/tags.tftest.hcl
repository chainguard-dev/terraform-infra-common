...
variables {
  ...
  name                  = "fixture"
  authorized-adder      = "serviceAccount:fixture@fixture-project.iam.gserviceaccount.com"
...
run "resource_manager_tags_bind_global_secret" {
  command = plan
  variables {
  ...
  assert {
    ...
    error_message = "global secret tag binding must preserve the tag value"
  }
  assert {
    ...
    error_message = "secret tag binding must use the numeric project number in the documented global full resource name"
  }
}
