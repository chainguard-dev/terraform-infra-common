# Regional Go cron jobs

An optional `name` on each `containers` entry sets its Cloud Run container name
only when the job is created. Omitting it lets Cloud Run assign the name.

The resource's `ignore_changes` preserves deployed container names. Setting or
changing `name` on an existing job does not rename its container or produce a
