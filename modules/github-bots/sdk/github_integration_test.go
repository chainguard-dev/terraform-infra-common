...
// NOTE: This is an integration test that requires 'GITHUB_TOKEN' env variable to be set!
// It is recommended to run this test in a local environment.
func Test_SearchContentInFilenameInRepository(t *testing.T) {
	ctx := context.Background()
	...
	// sdk allows an override of GIT_TOKEN env var so we can test from a local environment
	cli := NewGitHubClient(ctx, repoOrg, repoName, "test")
	result, err := cli.SearchContentInFilenameInRepository(ctx, repoOrg, repoName, "pyproject.toml", "py37", &github.ListOptions{})
	if err != nil {
		t.Fatalf("SearchContentInFilenameInRepository err: %v\n", err)
	}

	if *result.Total == 0 {
		t.Fatalf("SearchContentInFilenameInRepository result is zero\n")
	}
}
