...
| <a name="input_base_image"></a> [base\_image](#input\_base\_image) | The base image that will be used to build the container image. | `string` | `"cgr.dev/chainguard/static:latest-glibc@sha256:bf639cba19ba56329e6907ac26a7afcdde57a80b6aa66d5100da6883196e6b82"` | no |
...
| <a name="input_exec_triggers"></a> [exec\_triggers](#input\_exec\_triggers) | Values whose change re-runs the exec'd job. Empty (the default) re-runs on every container image change, i.e. every build. Set it when the job's work is keyed to something narrower — a schema file's hash, a list of grantees — so a rebuild that changes none of it does not re-execute (and, with exec\_wait, does not hold the apply). | `map(string)` | `{}` | no |
...
| <a name="input_failed_execution_alert_documentation"></a> [failed\_execution\_alert\_documentation](#input\_failed\_execution\_alert\_documentation) | Markdown documentation attached to the failed-execution alert. Shown in the incident and notification (e.g. a runbook or a Logs Explorer link). Empty (default) attaches none, which is the same shape as success\_alert\_documentation -- but the two alerts differ in who is likely to need it. A job with no fixed schedule reaches on-call only when someone ran it deliberately, so the receiver may be the one person who knows least about why it exists. | `string` | `""` | no |
...
| <a name="input_launch_stage"></a> [launch\_stage](#input\_launch\_stage) | The launch stage of the Cloud Run job (e.g. BETA to leverage features like disk volumes). | `string` | `"GA"` | no |
