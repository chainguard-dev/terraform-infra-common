...
import (
	...
	"github.com/chainguard-dev/terraform-infra-common/modules/zendesk-events/internal/secrets"
...
func main() {
	...
	// Get all secrets from the environment. Loads any env var prefixed with
	// WEBHOOK_SECRET, supporting rotation and multiple webhook subscriptions.
	webhookSecrets := secrets.LoadFromEnv(ctx)

	go httpmetrics.ServeMetrics()
	defer httpmetrics.SetupTracer(ctx)()

	ceclient, err := mce.NewClientHTTP("zendesk-trampoline", mce.WithTarget(ctx, env.IngressURI)...)
	if err != nil {
	...
	clog.FatalContextf(ctx, "ListenAndServe: %v", (&http.Server{
		...
		Handler:           httpmetrics.Handler("zendesk-trampoline", trampoline.NewServer(ceclient, webhookSecrets)),
	}).ListenAndServe())
}
