...
resource "google_service_account" "service" {
  ...
  display_name = "Service account for Zendesk events trampoline service"
}
...
output "recorder-schemas" {
  description = "Map of CloudEvent type to BigQuery table schema for the ticket.* events this module emits, for wiring into a cloudevent-recorder."
  value = {
    "dev.chainguard.zendesk.ticket.created" : {
      schema = file("${path.module}/schemas/ticket-created.schema.json")
    }
    "dev.chainguard.zendesk.ticket.status_changed" : {
      schema = file("${path.module}/schemas/ticket-status-changed.schema.json")
    }
    "dev.chainguard.zendesk.ticket.priority_changed" : {
      schema = file("${path.module}/schemas/ticket-priority-changed.schema.json")
    }
    "dev.chainguard.zendesk.ticket.comment_added" : {
      schema = file("${path.module}/schemas/ticket-comment-added.schema.json")
    }
  }
}
