...
variable "dev_users" {
  description = "List of developer email addresses granted OS Login & Cloud SQL access."
  type        = list(string)
}
...
variable "postgres_conn_name" {
  description = "Cloud SQL connection name <project>:<region>:<instance>. When empty, the Postgres proxy is not installed."
  type        = string
  default     = ""
}
