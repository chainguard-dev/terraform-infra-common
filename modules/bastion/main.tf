...
locals {
  ...
  bastion_startup = join("\n", compact([
    ...
    <<-COMMON
      ...
      # ------------------------------------------------------
      curl -sSO https://dl.google.com/cloudagents/google-cloud-ops-agent-installer
      bash google-cloud-ops-agent-installer --also-install

      # ------------------------------------------------------
      # Enable auditd to record all commands & auth events
      # ------------------------------------------------------
      apt-get update -y
      apt-get install -y auditd
      systemctl enable --now auditd.service

      cat > /etc/audit/rules.d/99-bastion.rules <<'AUDIT'
    ...
    var.postgres_conn_name != "" ? <<-POSTGRES
      # ------------------------------------------------------
      # Install Cloud SQL Auth Proxy v2 (PostgreSQL)
      # ------------------------------------------------------
      ...
      [Service]
      Type=simple
      ExecStart=/usr/local/bin/cloud-sql-proxy --private-ip --address 127.0.0.1 --port 5432 ${var.postgres_conn_name}
      Restart=always
      RestartSec=5
...
resource "google_project_iam_member" "bastion_sa_cloudsql_client" {
  count   = var.postgres_conn_name != "" ? 1 : 0
  project = var.project_id
  role    = "roles/cloudsql.client"
  member  = "serviceAccount:${google_service_account.bastion_sa.email}"
}

resource "google_project_iam_member" "bastion_sa_cloudsql_instance_user" {
  count   = var.postgres_conn_name != "" ? 1 : 0
  project = var.project_id
...
resource "google_project_iam_member" "dev_cloudsql_client" {
  for_each = var.postgres_conn_name != "" ? toset(var.dev_users) : toset([])
  project  = var.project_id
  role     = "roles/cloudsql.client"
  member   = "user:${each.key}"
}
...
// Ensure IAM DB user exists for SA (avoids first-login latency)
resource "google_sql_user" "bastion_sa_db_user" {
  count    = var.postgres_conn_name != "" ? 1 : 0
  project  = var.project_id
  instance = split(":", var.postgres_conn_name)[2]

  name = trimsuffix(google_service_account.bastion_sa.email, ".gserviceaccount.com")
